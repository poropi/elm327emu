...
guid Uuid16(uint16_t s) {
  return guid{static_cast<uint32_t>(static_cast<uint32_t>(s) << 16),
              0x0000,
              0x1000,
...
void BleGattServer::Start(bool use_fff0) {
  ...
  auto writeHandler =
      [this](GattLocalCharacteristic const&,
             GattWriteRequestedEventArgs const& args) -> IAsyncAction {
  ...
  if (notifyUuid == writeUuid) {
    ...
    wparams.CharacteristicProperties(
        GattCharacteristicProperties::Write |
        GattCharacteristicProperties::WriteWithoutResponse);
    auto wres = service.CreateCharacteristicAsync(writeUuid, wparams).get();
