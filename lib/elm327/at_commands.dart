...
class AtCommands {
  ...
  int _h(RegExpMatch m, [int group = 1]) =>
      int.parse(m.group(group)!, radix: 16);
  ...
  late final List<_Rule> _rules = [
    ...
    _Rule(
      '@2',
      (m, r) => state.deviceId == null ? _q : AtReply([state.deviceId!]),
    ),
    ...
    _Rule(
      'PP($_hx{2})ON',
      (m, r) => _set(() => _ppRange(_h(m)).forEach(state.ppEnabled.add)),
    ),
    _Rule(
      'PP($_hx{2})OFF',
      (m, r) => _set(() => _ppRange(_h(m)).forEach(state.ppEnabled.remove)),
    ),
    _Rule(
      'PP($_hx{2})SV($_hx{2})',
      (m, r) => _set(() => state.ppValues[_h(m)] = _h(m, 2)),
    ),
    ...
    _Rule(
      'RV',
      (m, r) => AtReply([
        '${(voltage() + (state.voltageOffset ?? 0)).toStringAsFixed(1)}V',
      ]),
    ),
    ...
    _Rule(
      'AT([012])',
      (m, r) => _set(
        () => state.timing = AdaptiveTiming.values[int.parse(m.group(1)!)],
      ),
    ),
    ...
    _Rule(
      'SP00',
      (m, r) => _set(() => state.setProtocol(0, auto: true, save: true)),
    ),
    _Rule(
      'SP([0-9A-C])',
      (m, r) => _set(() {
    ...
    _Rule(
      'TPA([0-9A-C])',
      (m, r) => _set(() => state.setProtocol(_h(m), auto: true, save: false)),
    ),
    _Rule('SS', (m, r) => _ok),
    _Rule(
      'ST($_hx{2})',
      (m, r) => _set(() => state.timeoutHex = _h(m) == 0 ? 0x32 : _h(m)),
    ),
    ...
    _Rule(
      'CS',
      (m, r) =>
          AtReply(['T:${hex2(state.txErrors)} R:${hex2(state.rxErrors)}']),
    ),
    ...
    _Rule(
      'FCSH($_hx{3}|$_hx{8})',
      (m, r) => _set(() => state.flowHeader = _h(m)),
    ),
    _Rule(
      'FCSD((?:$_hx{2}){1,5})',
      (m, r) => _set(() {
  ...
  Iterable<int> _ppRange(int n) =>
      n == 0xFF ? List.generate(0x30, (i) => i) : [n];
  ...
  AtOutcome _setFlowMode(int mode) {
    if (mode > 2) return _q;
    if (mode == 1 && (state.flowHeader == null || state.flowData == null)) {
      return _q;
    }
